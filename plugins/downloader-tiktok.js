...
const handler = async (m, {conn, text, args, usedPrefix, command}) => {
  ...
  try {
    ...
    const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: texto, contextInfo: {externalAdReply: {title: '「  𝑪𝒓𝒂𝒛𝒚 𝑩𝒐𝒕 」', body: null, thumbnail: imagen1, sourceUrl: 'https://instagram.com/oween._.001?igshid=MzMyNGUyNmU2YQ=='}, mentionedJid: [m.sender]}}}, aa);
    await conn.relayMessage(m.chat, prep.message, {messageId: prep.key.id, mentions: [m.sender]});
    const dataFn = await conn.getFile(`https://api-brunosobrino.zipponodes.xyz/api/tiktokv2?url=${args[0]}`);
    const desc1n = `*¡Hola Crazy! 🐼*\n*📽️ Aquí tienes tu video de TikTok.*\n\n*Recuerda que si deseas convertir el video en audio, puedes hacerlo fácilmente respondiendo al video con el comando #tomp3 🎧.*`;
...
handler.command = /^(tiktok|ttdl|tiktokdl|tiktoknowm|ttk|ttnowm|tiktokaudio)$/i;
